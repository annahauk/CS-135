...
;; 1 PTS
(define (pred p)
  (if (null? p) (error-predecessor) (car p))
)
...
(define (p-prime? p)
  (define (div a)
    (cond
      ...
      [(pz? (p% p a)) #f]
      [else (div (succ a))]))
  (if (or (pz? p) (pz? (pred p))) #f (div (succ (succ z))))
)



"(cond
    [(= (cadr current) g) (reverse current)] ;;step 1 
    [(= (cadr current) cB) (waterjug cA cB g (cons (cons (car current) 0) current))] ;; step 2
    [(= (car current) 0) (waterjug cA cB g (cons (cons cA (cadr current)) current))] ;; step 3
    [(> (+ (car current) (cadr current)) cB) (waterjug cA cB g (- cB (cadr (current))

                                                       (cons (cons (- (car current) amount) cB) current))] ;;b capacity - current val in B, subtract that < from however much is in a
    [(<= (+ (car current) (cadr current)) cB)
    [(> (+ (car current) (cadr current)) cB) (let ([amount (- cB (cadr current))]) (waterjug cA cB g (cons (cons (- (car current) amount) cB) current)))]
    [else (waterjug cA cB g (cons (cons 0 (+ (car current) (cadr current))) current))])
))"