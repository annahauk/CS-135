...
;; 5 PTS
(define (compose S R)
  (define (compose-helper S R)
    (cond
      [(null? S) '()]
      [(null? R) '()]
      [(null? (relate (cadar R) S)) (compose-helper S (cdr R))]
      [else (append (map (lambda (s) (list  (caar R) s)) (relate (cadar R) S) ) (compose-helper S (cdr R)))]
    )
  )
  (make-set (compose-helper S R))
)
...
(define (transitive-closure R)
  (define (transitive-closure-helper k R)
    (cond
      [(null? R) '()]
      [(= k 0) '()]
      [else (union (power k R) (transitive-closure-helper (- k 1) R) )]
    )
  )
  (transitive-closure-helper (cardinality R) R)
)
...
(define (getTransitives R)
    (make-set (flatten-once (map (lambda (t) (compose (difference R (list t)) (list t))) R))))
(define (transitive? R)
    (if (< (cardinality R) 2) #t
    (set-equal? (intersection (getTransitives R) R) (getTransitives R)))
)
...
(define (equivalence? n R)
   (and (reflexive? n R) (symmetric? R) (transitive? R))
 )
#| OPTIONAL:
...
(define (involutive? n R)
   (and (injective? R) (surjective? n R) (symmetric? R))  
)
...
(define (surjective? n R)
  (= (cardinality (id-sequence n)) (cardinality R)))

(define (id-sequence n)
  (if (= n 0)'() (cons n (id-sequence (- n 1)))))
