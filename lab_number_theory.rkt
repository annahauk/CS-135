...
;; 5 PTS
(define (waterjug cA cB g [current '((0 0))])
  (cond
    [(= (cadr (last current)) g) current] ;;step 1
    [(= (cadr (last current)) cB) (waterjug cA cB g (append current (list (list (car (last current)) 0))))] ;;step 2
    [(= (car (last current)) 0) (waterjug cA cB g (append current (list (cons cA (cdr (last current))))))] ;;step 3
    [(> (+ (car (last current)) (cadr (last current))) cB) (waterjug cA cB g (append current (list (list (- (car (last current)) (- cB (cadr (last current)))) cB))))] ;;Step 4
    [else (waterjug cA cB g (append current (list (list 0 (+ (car (last current)) (cadr (last current)))))))]
))
...
;; 5 PTS
(define (factorize n [l '()])
  (cond
    [(= n 0) '(0)]
    [(= n 1) (if (null? l) '(1) l)]
    [else (factorize (/ n (smol n 2)) (append l (list (smol n 2))))])
)
(define (smol n p)
  (if (= (modulo n p) 0) p (smol n (+ p 1)))
  )
