...
;; 3 PTS
"use element? help func"
"COME BACK"
(define (intersection S1 S2)
  (cond
   [(null? S1) '()]
   [(element? S2 (car S1)) (cons (car S1) (intersection (cdr S1) S2))]
...
(define (disjoint? S1 S2)
  (cond
   [(null? S1) #t]
   [(element? S1 (car S2)) #f]
   [(not (element? S1 (car S2))) #t]
   [else (disjoint? S1 (cdr S2))]
)
  )
...
(define (subset? S1 S2)
  (cond
    [(null? S1) #t]
    [(element? S2 (car S1)) (subset? (cdr S1) S2)]
    [else #f]
    )
)
...
"(if (not (equal? (length S1) (length S2))) #f #f)"
...
#| Implement "difference" to accept sets S1 and S2
 ...
 |   (difference '("xyz") '())      -> ("xyz")
 |#
;; subset: element in S1 is also in S2
...
;; 2 PTS
(define (symmetric? R)
  (set-equal? R (union (converse R) R))
)
