...
(define (generate p q e)
  (append (list (list (* p q) e)) (list (list (* p q) (mod-inverse e (lcm (- p 1) (- q 1))))))
)
...
(define (mod-exp b e m)
  (cond
  [(= e 0) 1]
  [(= (modulo e 2) 0) (modulo (expt (mod-exp b (/ e 2) m) 2) m)]
...
(define (encrypt plaintext key)
  (define m (car key))
  (define e (cadr key))
  (map (lambda (c) (mod-exp c e m)) (map char->integer (string->list plaintext)))
)
...
(define (decrypt ciphertext key)
  (define m (car key))
  (define d (cadr key))
  (list->string (map integer->char (map (lambda (n) (mod-exp n d m)) ciphertext))))
