...
(define (pulverizer a b)
  (if (= b 0)
      (list 1 0)
      (gcd-list (pulverizer b (modulo a b)) (quotient a b))))

(define (gcd-list lst q)
  (list (cadr lst) (- (car lst) (* q (cadr lst)))))

" first checks if b is equal to 0, if so returns list 1,0
;; ri = ai-qibi
;; b not 0? it calculates the quotient of a/b and the remainder of a/b, then calls recursively with b while storing remainders in a list
;;ti, si - si(ai-1/bi-1)
;; ai = bi -1
;;bi = rem(ai-1, bi-1)
;; finally takes first element of last pair in list for the x coeff
;; and the second element (y coeff) is calculated -q * x + y, where q is the quotient of a/b
;; I used this site to better understand the algorithm but the code of my work came from myself and my recitation:
 https://www.claritician.com/extended-euclidean-algorithm-a-k-a-the-pulverizer"
...
(define (mod-inverse a m)
  (if (not (= (gcd a m) 1)) (error-inverse) (modulo (cadr (pulverizer m a)) m))
)