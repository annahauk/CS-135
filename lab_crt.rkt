...
;;
(define (find-m L)
   (if (null? L) 1 (* (cadar L) (find-m (cdr L))))
  )
...
;; 4 PTS
;;see if every different combo of gcd is equal 1 or
;;something with the product of all of them

(define (CRT-help L a)
  (cond
    [(null? L) #t]; base case: an empty list is pairwise relatively prime
    [(= (gcd (cadar L) a) 1) (CRT-help (cdr L) a)] ; check if gcd of first pair and second pair is 1
    [else #f])
)

;; Main function to check if CRT is possible
(define (CRT-exists? L)
  (cond
    [(null? L) #t] ; base case: an empty list is not possible for CRT
    [(not (CRT-help (cdr L) (cadar L))) #f] ; check if all numbers inside the mod (b) are pairwise relatively prime
    [else (CRT-exists? (cdr L))])
  ) ; if not, CRT is not possible
...
(define (CRT-helper L m)
  (cond
    [(null? L) 0]
    [else (+ (* (caar L) (/ m (cadar L)) (mod-inverse (/ m (cadar L)) (cadar L))) (CRT-helper (cdr L) m))])
)
...
(define (CRT L)
  (cond
    [(not (CRT-exists? L)) -1]
    [else (modulo (CRT-helper L (find-m L)) (find-m L))]
    )
)