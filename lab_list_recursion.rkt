...
;; 2 PTS
(define (rotate c)
  (define ro1 (list (cadr c) (- 0 (car c))))
  (define ro2 (list (- 0 (car c)) (- 0 (cadr c))))
  (define ro3 (list (- 0 (cadr c)) (car c)))
  (list c ro1 ro2 ro3)
)
...
"DONE"
(define (get-state s)
  (car (cdr (car (reverse (get-address s)))))
)
...
(define (combine-courses s1 s2)
  (append (get-courses stu1) (get-courses stu2))
)
...
"DONE"
(define (sign x)
  (if (zero? x) 'zero (if (positive? x) 'positive 'negative))
)
...
;; find last returns last element of a list
;;(define (findlast L)
;;  (cond ((null? L) 'ERROR_EMPTY_LIST ) ((null? (cdr L)) (car L)) (else (findlast (cdr L))))
;;)
...
;; 3 PTS
(define (sum-pow p n)
  (if (zero? n) 0 (+ (expt n p) (sum-pow p (- n 1))))
)
...
(define (subfact n)
  (cond [(= 1 n) 0]
        [(= 2 n) 1]
        [(and (< 2 n) (even? n)) (+ 1 (* n (subfact(- n 1))))]
        [(and (< 2 n) (odd? n)) (- (* n (subfact(- n 1))) 1)])
)
...
;; call cdr again because multiplying one too many times 
...
;; 4 PTS
(define (filter P L)
  (if (null? L) empty (if (P (car L)) (append (list (car L)) (filter P (cdr L))) (filter P (cdr L) )))
)